...
export default class WalletScreen extends Component {
  ...
  componentDidMount() {
    ...
    axios({
      method: "POST",
      url: `http://localhost:5000/api/u/wallet/jobs`, // First page at 0
      data: data,
      headers: {
    ...
    return (
      <div className={"wallethome"}>
        <div className={"walletround"}>
          Pending Amount
          <br />
          <br />R {this.state.pending}
          <br />
          <br />
          Total Amount
          <br />
          <br />R {this.state.total}
        </div>
        <List>
          {this.state.wallet.map((item) => (
            <div className={"walletlistdiv"}>
              <ListItem>
                ...
                />
                <div style={{ marginLeft: 30, marginTop: 10, fontSize: 26 }}>
                  {item.campaignName}{" "}
                </div>
                <div style={{ marginLeft: 20, marginTop: 10, fontSize: 23 }}>
                  {item.amount}
                </div>
                <div style={{ marginLeft: 20, marginTop: 10, fontSize: 23 }}>
                  {item.status}
