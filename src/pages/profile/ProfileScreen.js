...
function ProfileScreenmain(props) {
  ...
  const onSub = () => {
    ...
    axios({
      method: "POST",
      url: `http://localhost:5000/api/u/settings/bio`, // First page at 0
      data: data,
      headers: {
  ...
  const onUpload = () => {
    ...
    axios({
      method: "POST",
      url: `http://localhost:5000/api/u/profile-pic`, // First page at 0
      data: form,
      headers: {
        Authorization: `Bearer ${sessionStorage.getItem("token")}`,
      },
        ...
        <div className={"probox"}>
          <div className={"toppro"}>
            <ListItem style={{ width: "70%" }}>
              ...
              <ListItemIcon>
                <StyledButton2 onClick={onEdit}>{buttons}</StyledButton2>
              </ListItemIcon>
            </ListItem>
            <img
              src={!changed ? profile.image : image}
              className="imge"
              alt={image}
            />
            {edit && (
              <div className={"rowsz"}>
                <input
                  ...
                  id="customFile"
                />
                <StyledButton2 onClick={onUpload}>upload</StyledButton2>{" "}
              </div>
            )}
            <Rating
                ...
                <Tab eventKey="Profile" title="About">
                  <div className={"tabs"}>
                    <div className={"inputedits"}>
                      <div style={{ color: "white" }} className={"identi"}>
                        Full Names:
                      </div>
                      <div style={{ color: "white" }} className={"identi"}>
    ...
    >
      <div style={{ height: 200, width: 300 }}>
        <div>
          ...
          <StyledButton style={{ top: 10, bottom: 10 }}>Save</StyledButton>
          <h3 style={{ color: "transparent" }}>function dfffffff</h3>
        </div>
      </div>
    </Menu>
  ...
  return (
    <div className={classes.root}>
      <GridList cellHeight={180} className={classes.gridList}>
        ...
        <GridListTile>
          <button onClick={handleBankMenuOpen} className={"buttns"}>
            +
          </button>
        </GridListTile>
      </GridList>
...
export default class ProfileScreen extends React.Component {
  ...
  componentWillMount() {
    ...
    axios({
      method: "POST",
      url: `http://localhost:5000/api/u`, // First page at 0
      data: data,
  ...
  render() {
    return <ProfileScreenmain window={window} profile={this.state.details} />;
  }
}
