  ...
  const EditDet = (props) => {
    ...
    const onSubmit = (event) => {
      ...
      var data = {
        ...
        name: first === "" ? Settings.name : first,
      ...
      axios({
        method: "POST",
        url: `http://localhost:5000/api/u/update`, // First page at 0
        data: data,
        headers: {
      ...
      <div style={{ width: "90%" }} className={"divs"}>
        ...
        <div className={"inputedit"}>
          ...
          <input
            type="date"
            value={
              typeof Settings.dob === "undefined"
                ? ""
                : Settings.dob.substring(0, 10)
            }
            name="date"
            onChange={handleChanges}
            variant="outlined"
      ...
      <div className={"settingshid"}>
        <Switch>
          <Route
            ...
            render={(props) => <EditDet handle={handleEditOut} />}
          />
          <Route
            exact
...
function EditPayments(props) {
  ...
  const handleBank = (event) => {
    ...
    var data = {
      bankName: name === "" ? Settings.bankName : name,
    ...
    axios({
      method: "POST",
      url: `http://localhost:5000/api/u/settings/banking`, // First page at 0
      data: data,
      headers: {
        Authorization: `Bearer ${sessionStorage.getItem("token")}`,
      },
  ...
  const handleMomo = (event) => {
    ...
    axios({
      method: "POST",
      url: `http://localhost:5000/api/u/settings/momo`, // First page at 0
      data: data,
      headers: {
        Authorization: `Bearer ${sessionStorage.getItem("token")}`,
      },
      ...
      {edit && (
        <div style={{ width: "80%" }}>
          <div style={{ fontSize: 20 }}>
            Longa Money will use the bank account information below to ensure
            you receive payment for Campaings you participate in. Make sure the
            details below are correct and up to date. These details are stored
            securely in our system and will never be shared with third parties.
          </div>
          <div className={"inputedit"}>
          ...
          <div className={"inputedit"}>
            ...
            <input
              ...
              type="text"
              name="aty"
              defaultValue={Settings.accountType}
              variant="outlined"
              className="setedit"
          ...
          <div className={"placebtn"}>
            ...
            <ListItem>
              ...
              <ListItemIcon>
                <StyledButton onClick={handleBank}>Save</StyledButton>
              </ListItemIcon>
            </ListItem>
          </div>
        </div>
        ...
        <div style={{ width: "80%" }}>
          <div className={"inputedit"}>
            ...
            <input
              ...
              onChange={handleChange}
              name="contactNumber"
              defaultValue={props.settings.momo}
              variant="outlined"
              className="setedit"
              placeholder="+27 83X XXX XXX"
            />
          </div>
          {renderMenu}
          <div className={"placebtn"}>
            <ListItem>
              <ListItemText primary={""} />
              <ListItemIcon>
                <StyledButton onClick={handleMomo}>Save</StyledButton>
              </ListItemIcon>
            </ListItem>
          </div>
        </div>
...
function EditSocials(props) {
  ...
  const AddInsta = (event) => {
    ...
    window.open(
      `http://localhost:5000/api/i/auth/insta/${data.userID}`,
      "_self"
    );
    // axios({
    //   method: "GET",
    //   url: `http://localhost:5000/api/i/auth/insta/${data.userID}`, // First page at 0
    //   data: data,
    //   headers: {
    //     Authorization: `Bearer ${sessionStorage.getItem("token")}`,
    //   },
  ...
  const AddYoutube = (event) => {
    ...
    console.log(data);
    window.open(`http://localhost:5000/api/y/auth/yt/${data.userID}`, "_self");
    // axios({
    //   method: "GET",
    //   url: `http://localhost:5000/api/y`, // First page at 0
    //   data: data,
    //   headers: {
    //     Authorization: `Bearer ${sessionStorage.getItem("token")}`,
    //   },
  ...
  const AddTwitter = (event) => {
    ...
    window.open(`http://localhost:5000/api/t/auth/tw/${data.userID}`, "_self");
    // axios({
    //   method: "GET",
    //   url: `http://localhost:5000/api/t/auth/twitter/${data.userID}`, // First page at 0
    //   data: data,
    //   headers: {
    //     Authorization: `Bearer ${sessionStorage.getItem("token")}`,
    //   },
  ...
  const AddFacebook = (event) => {
    ...
    window.open(`http://localhost:5000/api/fb/auth/fb/${data.userID}`, "_self");
    // axios({
    //   method: "POST",
    //   url: `http://localhost:5000/api/fb`, // First page at 0
    //   data: data,
    //   headers: {
    //     Authorization: `Bearer ${sessionStorage.getItem("token")}`,
    //   },
      ...
      {!editTwitter && !editInsta && !editYoutube && (
        <div>
          <ListItem button style={{ width: "85%" }} onClick={handleFacebook}>
            ...
            <ListItemText
              style={{ marginLeft: 6 }}
              primary={"Add/Edit Facebook account"}
            />
            <ListItemIcon>
      ...
      {!editTwitter && !editYoutube && !editFacebook && (
        <div>
          <ListItem button style={{ width: "85%" }} onClick={handleInsta}>
            ...
            <ListItemText
              style={{ marginLeft: 6 }}
              primary={"Add/Edit Instagram Account"}
            />
            <ListItemIcon>
              <IconButton edge="end" aria-label="delete">
                <CreateIcon />
      ...
      {!editInsta && !editYoutube && !editFacebook && (
        <div>
          <ListItem button style={{ width: "85%" }} onClick={handleTwitter}>
            ...
            <ListItemText
              style={{ marginLeft: 6 }}
              primary={"Add/Edit Twitter Account"}
            />
            <ListItemIcon>
              <IconButton edge="end" aria-label="delete">
                <CreateIcon />
      ...
      {!editTwitter && !editInsta && !editFacebook && (
        <div>
          <ListItem button style={{ width: "85%" }} onClick={handleYoutube}>
            ...
            <ListItemText
              style={{ marginLeft: 6 }}
              primary={"Add/Edit Youtube Channel"}
            />
            <ListItemIcon>
              <IconButton edge="end" aria-label="youtu">
                <CreateIcon />
...
function EditAccount(props) {
  ...
  const handleUpdate = () => {
    ...
    if (!valid) {
      axios({
        method: "POST",
        url: `http://localhost:5000/api/u/settings/change-password`, // First page at 0
        data: data,
        headers: {
          Authorization: `Bearer ${sessionStorage.getItem("token")}`,
        },
  ...
  const handleDelete = () => {
    ...
    axios({
      method: "POST",
      url: `http://localhost:5000/api/u/delete`, // First page at 0
      data: data,
      headers: {
        Authorization: `Bearer ${sessionStorage.getItem("token")}`,
      },
    ...
    >
      ...
      >
        <div className={"delText"}>
          Are you sure you want delete your Longa Money account? Other
          influencers will not be able to see your profile or communicate with
          you.
        </div>
        <div className={"rowx"}>
          <StyledButton
    ...
    <div style={{ width: "97%" }} className={"divs"}>
      ...
      {editYoutube && (
        <div className={"columnx"}>
          ...
          <div className={"placebtn"}>
            {valid && <div style={{ color: "red" }}>{error}</div>}
...
export default class Settings extends React.Component {
  ...
  componentWillMount() {
    ...
    axios({
      method: "POST",
      url: `http://localhost:5000/api/u/settings`, // First page at 0
      data: data,
      headers: {
        Authorization: `Bearer ${sessionStorage.getItem("token")}`,
      },
